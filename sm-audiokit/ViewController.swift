...
//            let channelName = "guitar"
        ...
        do {
            try AudioManager.shared.setBrowserTime(0.1)
            try AudioManager.shared.playSample(sampleId: randomSample.id, channel: "guitar", playbackId: UUID().uuidString, atTime: 0.2)
    ...
    @IBAction func tappedScheduled200ms(_ sender: Any) {
        ...
        do {
            try AudioManager.shared.playSample(sampleId: testTone.id, channel: "test", playbackId: UUID().uuidString, atTime: 0.0)
    ...
    @IBAction func tappedScheduledSample(_ sender: Any) {
        ...
        do {
            ...
            do {
                ...
                for beat in 0...31 {
                    if beat % 4 == 0 {
                        try AudioManager.shared.playSample(sampleId: "kick", channel: "test", playbackId: ("kick"+String(beat)), atTime: 0.2 + (Float(beat) / bpm))
                    }
                    if beat % 4 == 2 {
                        try AudioManager.shared.playSample(sampleId: "snare", channel: "test", playbackId: ("snare"+String(beat)), atTime: 0.2 + (Float(beat) / bpm))
                    }
                    if beat % 8 != 7 {
                        try AudioManager.shared.playSample(sampleId: "hat-closed", channel: "test", playbackId: ("hat"+String(beat)), atTime: 0.2 + (Float(beat) / bpm))
                    } else {
                        try AudioManager.shared.playSample(sampleId: "hat-open", channel: "test", playbackId: "hat-open-pb", atTime: 0.2 + (Float(beat) / bpm))
